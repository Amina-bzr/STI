...
def recherche_elem_suiv(request):
    if request.method == 'POST':  # si le switch d'id = switch_id n'existe pas on renvoie 404
        ...
        if form.is_valid():
            ...
            if n != "":
                port = Port.objects.filter(nom_suiv=n)
                print(port)
                if port.exists():
                ...
                po = []
                port = Port.objects.filter(local=l)
...
def portConfig(request, switch_id, port_num):
    ...
    if request.method == 'POST':  # si le switch d'id = switch_id n'existe pas on renvoie 404
        ...
        if form.is_valid():
            ...
            messages.success(
                request, ('port numero '+str(p.num_port)+' configuré avec succés!'))
            return redirect('../port_tab/', switch_id)
        else:
            messages.warning(
                request, ('echec lors de la configuration, veuillez réessayer une autre fois.'))
    else:
...
def vlan_tab(request):
    cols_principales = ['Vlan ', 'Nom', 'Adresse réseau',
                        'ip', 'Masque Sous Réseau', 'Passerelle', ' ', ' ']
    cols_detail = []
