...
def recherche_elem_suiv(request):
	...
	if request.method == 'POST':  # si le switch d'id = switch_id n'existe pas on renvoie 404
		if form.is_valid():
			...
			t= form.cleaned_data["type_suiv"]
			
			if ((n!="" and  l!="") or(n!="" and t!="")):
				 messages.warning(
						request, ("Veuillez completer qu'un seul champ.. si vous conaissez le nom de l'equipement il n'est pas question d'introduire le local ou le type de cet equipement." ))
			
			elif (l!=""and t!=""):
				i = 0
				po = []
				port_l= Port.objects.filter(local__iexact=l)
				if port_l.exists():
					for p in port_l.all():
					...
					if (i==0):
						...
						messages.warning(
							request, ("Aucun port n'est relié à un équipement de ce type qui se trouve dans ce local."))
					else:
				...
				if  port_t.exists() :
					...
					messages.warning(
						request, ("Aucun port n'est relié à un équipement de ce type."))
...
def portConfig(request, switch_id, port_num):
	...
	return render(request,
				  'app_principal/form_validation.html',
				  {'form': form, 'choix': 'Port', 'switch':s, 'objet':' du Port '+str(p.num_port)+' de '+s.nom, 'operation': 'Configuration', })
